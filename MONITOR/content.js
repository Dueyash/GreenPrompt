...
function init() {
    ...
    console.log('[Green Prompt] Initializing on DeepSeek chat page');
...
async function exportChat(format) {
  try {
    const chatData = await chatDB.getChatData();
    
    if (!chatData.messages || chatData.messages.length === 0) {
      alert(chrome.i18n.getMessage('noMessagesFound') || 'No messages found in database');
      return;
    }
    ...
    const formattedData = {
      ...
      messages: exportData.messages.map(msg => {
        ...
        if (msg.role === 'assistant' && msg.chain_of_thought) {
          formattedMsg.chain_of_thought = typeof msg.chain_of_thought === 'string' 
...
function convertToMarkdown(data) {
  ...
  data.messages.forEach((msg, index) => {
    ...
    if (msg.role === 'assistant' && msg.chain_of_thought) {
      ...
      const cotContent = typeof msg.chain_of_thought === 'string' ? msg.chain_of_thought : extractParagraphs(msg.chain_of_thought);
      markdown += `${cotContent}\n\n`;
      markdown += `</details>\n\n`;
    }
    ...
    if (msg.role === 'assistant') {
      const formattedContent = typeof msg.content === 'string' ? msg.content : domToMarkdown(msg.content);
...
function convertToPlainText(data) {
  ...
  data.messages.forEach((msg, index) => {
    ...
    const content = msg.role === 'user' ? msg.content : (typeof msg.content === 'string' ? msg.content : domToMarkdown(msg.content));
    text += `${content}\n\n`;
...
`;
  ...
  data.messages.forEach(msg => {
    ...
    if (msg.chain_of_thought) {
      let cotContent = msg.chain_of_thought;
      if (typeof msg.chain_of_thought === 'string') {
        cotContent = msg.chain_of_thought.replace(/\n/g, '<br>');
      } else {
        cotContent = msg.chain_of_thought.innerHTML;
      }
...
function convertToJSON(formattedData) {
  const messages = formattedData.messages.map(msg => {
    if (msg.role === 'assistant') {
      return {
        role: msg.role,
        content: typeof msg.content === 'string' ? msg.content : domToMarkdown(msg.content),
        chain_of_thought: typeof msg.chain_of_thought === 'string' 
          ? msg.chain_of_thought 
          : extractParagraphs(msg.chain_of_thought),
      };
    }
    return {
      role: msg.role,
...
function startRealtimeMonitoring() {
  console.log('[Green Prompt] Starting real-time monitoring...');
  
  saveCurrentPageMessages();
      ...
      if (mutation.type === 'childList') {
        const hasRelevantChanges = Array.from(mutation.addedNodes).some(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            return node.classList?.contains('fbb737a4') || 
                   node.classList?.contains('ds-message') ||
                   node.querySelector?.('.fbb737a4, .ds-message');
          }
          return false;
        });
  ...
  console.log('[Green Prompt] ✅ Real-time monitoring active - Messages will be auto-saved');
}

async function saveCurrentPageMessages() {
  try {
    const { messages } = extractAllMessagesFromPage();
    if (messages && messages.length > 0) {
      console.log(`[Green Prompt] 💾 Saving ${messages.length} existing messages...`);
      for (const msg of messages) {
        await saveMessageToDatabase(msg);
      }
    }
  } catch (error) {
    console.error('[Green Prompt] ❌ Error saving current messages:', error);
  }
}
...
async function processNewMessages() {
  try {
    ...
    for (const msg of messages) {
      if (!processedElements.has(msg.element_id)) {
        await saveMessageToDatabase(msg);
        processedElements.add(msg.element_id);
      }
    }
  } catch (error) {
    console.error('[Green Prompt] ❌ Error processing new messages:', error);
  }
}
...
async function saveMessageToDatabase(message) {
  try {
    ...
    if (message.role === 'assistant') {
      if (typeof message.content === 'object' && message.content.nodeType) {
        messageData.content = extractMessageContent(message.content);
      } else {
        messageData.content = message.content;
      }
      
      if (message.chain_of_thought) {
        messageData.chain_of_thought = extractThinkingContent(message.chain_of_thought);
      }
    } else {
      messageData.content = message.content;
    }

    const saved = await chatDB.saveMessage(messageData);
    if (saved) {
      console.log(`[Green Prompt] ✅ Saved ${message.role} message`);
    }
  } catch (error) {
    console.error('[Green Prompt] ❌ Error saving message:', error);
  }
}

function extractMessageContent(domElement) {
  if (!domElement) return '';
  if (typeof domElement === 'string') return domElement;
  return domToMarkdown(domElement);
}

function extractThinkingContent(domElement) {
  if (!domElement) return '';
  if (typeof domElement === 'string') return domElement;
  return extractParagraphs(domElement);
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getStats') {
    chatDB.getStats().then(stats => {
      ...
      console.error('[Green Prompt] Error getting stats:', error);
      sendResponse({ totalChats: 0, totalMessages: 0, error: error.message });
    });
    return true;
  } else if (request.action === 'getAllChats') {
    chatDB.getAllChats().then(chats => {
      sendResponse({ chats });
    }).catch(error => {
      console.error('[Green Prompt] Error getting chats:', error);
      sendResponse({ chats: [], error: error.message });
    });
    return true;
  } else if (request.action === 'exportAllData') {
    chatDB.exportDatabase().then(data => {
      sendResponse({ data });
    }).catch(error => {
      console.error('[Green Prompt] Error exporting database:', error);
      sendResponse({ data: null, error: error.message });
    });
    return true;
  } else if (request.action === 'exportChat') {
    exportChat(request.format).then(() => {
      sendResponse({ success: true });
    }).catch(error => {
      console.error('[Green Prompt] Error exporting chat:', error);
